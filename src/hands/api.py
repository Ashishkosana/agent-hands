...
def create_api(
    ...
    def invoke(name: str) -> Any:
        ...
        # The idempotency check, the execution, and the record are ONE
        # critical section. Checking outside the lock is a check-then-act
        # race: two concurrent requests carrying the same key both read an
        # empty map, both pass, and the lock then politely serializes two
        # transfers instead of preventing the second one. A duplicate is
        # exactly the double-click this key exists to absorb, so the second
        # caller waits for the first to finish and receives its envelope — it
        # never executes. (Payload binding and restart durability of this map
        # are deliberately NOT addressed here — see the F9 backlog entry.)
        with _INVOKE_LOCK:
            ...
            # ONLY engine.run is guarded. A wider net would let an internal
            # fault raise ValueError, get reported as a 400 "your parameters
            # are wrong", and skip the `completed` write below — so the caller
            # "corrects" the request, retries the same key, and re-executes.
            try:
            ...
            envelope = {
                ...
                # or `hands explain <run> --expect-tip <tip>`.
