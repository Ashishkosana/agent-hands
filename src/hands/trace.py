...
and verification then silently degrades to the weaker chain-only check. Two
things make it real: ``is_sealed`` exposes presence as a REPORTED state
(unsealed must never render as "intact"), and the tip travels out of the
directory in the invoke envelope as ``audit_chain_tip``, to be handed back as
``verify_chain(dir, expected_tip=...)``. The external anchor is the actual
control; the file is a convenience.
...
Compatibility: traces written before sealing existed (the committed twin
evidence bundles) have no ``trace.tip``; they verify on the chain alone and
report ``is_sealed(...) == False``. Twin manifests hash ``trace.jsonl`` after
close, which the seal file does not alter.
"""
...
def verify_chain(run_dir: Path, expected_tip: str | None = None) -> bool:
    ...
    If a seal (``trace.tip``) is present it is cross-checked: the record count
    and the tip hash must match, which is what catches a rewritten final
    record or a truncated tail. ``expected_tip`` is the anchor a caller kept
    elsewhere (the invoke envelope's ``audit_chain_tip``); supplying it is the
    only way to detect a deleted seal or a consistently rewritten pair.
    Without it, an unsealed run verifies on the chain alone — check
    ``is_sealed`` and report that state rather than reading True as intact."""
    ...
    sealed = _read_seal(run_dir)
    if sealed is not None and (sealed.get("records") != len(lines) or sealed.get("tip") != prev):
        return False
